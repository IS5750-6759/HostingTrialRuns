...
const getData = async () => {
  await wait(5000);
  const { data } = await axios.get("http://144.24.55.88:3000/playlists");
  return data;
}

export const loader = async ({ params }) => {
  const { id } = params;
  const { data } = await axios.get(`http://144.24.55.88:3000/videos/${id}`);
...
export const action = async ({ params, request }) => {
  ...
  if (isValidUrl(youtubeUrl)) {
    try {
      await axios.put("http://144.24.55.88:3000/videos/" + id, { id, title, youtubeUrl, playlistId })
